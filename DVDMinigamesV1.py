...
def mainMenu():
    ...
    while active:
        ...
        #p.draw.rect(gameDisplay,(200,14,150), p.Rect(0,0,50,20))
        if((cursorX > 0 and cursorX < 50) and (cursorY > 0 and cursorY < 20) and click):
            nextMode = 0 #return to main menu
        #p.draw.rect(gameDisplay,(200,14,150), p.Rect(DISPLAY_WIDTH/2-50,DISPLAY_HEIGHT/2-25,100,50)) #(xStart, yStart, width, height)
...
def classicMode():
    ...
    goMain = False
    while run:
     #MAIN MENU | implement some sort of main menu.
     #main_menu()
    ...
    while run:
        ...
        if c.visible == True:
            p.draw.circle(gameDisplay, (122,12,11), (randomX - c.radius,randomY - c.radius), 20)
             #addCoin()
        #print(randomX)
        if(x > randomX-17 and x < randomX+10) and (y > randomY-10 and y < randomY+10):
            c.visible = False
            randomX = random.randint(0,DISPLAY_WIDTH)
            randomY = random.randint(0,DISPLAY_HEIGHT)

            #p.draw.circle(gameDisplay, (122,12,11), (100,100), 20)
         #print(seconds)
        for event in p.event.get():
            if event.type == p.QUIT:
                run = False
...
def mazeMode():
    ...
    while run:
        ...
        #p.draw.line(gameDisplay,(100,88,30),(50,0),(50,100))
        ...
        #print(K_c)
        #border conditions
        #switchX and switchY which will fliccur between 1 and -1
        if x > DISPLAY_WIDTH-(RESCALE_W/2)+17: #Right
            forward = False
